Track 3 - Lab 21 - Fine-tune LLM bằng LoRA
...
Ticket CSKH tiếng Việt → JSON 4 trường (intent, urgency, product, sentiment) — thang đo khách quan, không cần LLM-as-judge.
Nhìn loss mask bằng mắt: giải mã token được supervise, khẳng định câu trả lời nằm trong loss và câu hỏi không nằm trong loss.
Đóng băng eval + đo (a) và (b) TRƯỚC khi train, để không vô thức chỉnh prompt cho fine-tune trông thắng.
Train cấu hình ‘không hối tiếc’: all-linear text decoder, LR ≈ 10× full-FT, batch hiệu dụng < 32.
Chạy 3 đối chứng cùng step: attn_only (rank khớp ngân sách), wrong_lr, qlora — mỗi run chỉ một biến.
Phán quyết bốn nhóm (target · regression · format · latency) so với (b). Viết report kể cả ca fine-tune thua.
...
VLearn Codelabs | Học AI thực chiến qua LabTrack 3 - Lab 21 - Fine-tune LLM bằng LoRA
...
Mask đúng bằng mắt, không bằng niềm tin. results/mask_proof.json có answer_is_supervised: true, question_is_masked: true, và supervised_fraction < 0.95. Bạn dán được 3–5 dòng đầu của đoạn được tính loss vào REPORT.md.
Mốc đánh giá bị đóng băng trước khi train. results/baselines_frozen.json có điểm (a) và (b); (b).target > (a).target; bạn không sửa tập eval sau đó.
Có một bản LoRA “đúng cấu hình” trên đĩa. Thư mục adapters/correct/ chứa adapter_model.safetensors + adapter_config.json; results/runs.csv có dòng correct với loss, VRAM, max_steps.
Ba đối chứng công bằng. Cùng max_steps với correct; attn_only lệch ngân sách tham số < 5%; mỗi run chỉ một biến (vị trí / LR / 4-bit).
Phán quyết bốn nhóm. results/verdict.json ghi PASSED hoặc FAILED; results/autopsy.json xếp 4 run bằng điểm target, không bằng final_loss; results/qualitative.json có cả ca thắng lẫn ca thua.
Report khớp số. submission/REPORT.md đủ 7 mục, không còn placeholder <điền>, mọi con số lấy từ results/, kết luận ≥150 từ, ≥2 ví dụ fine-tune thua.
python scripts/verify.py (hoặc make verify) thoát mã 0. FAIL phải sửa; WARN phải giải thích trong report.
...
Bản fine-tune có thắng base đã prompt tử tế không — và bạn có phát hiện được nếu nó không thắng? (NB2 đóng băng, NB5 phán quyết)
...
Repo	Day21-Track3-Finetuning-Lab	Lab đi kèm deck day21-fine-tuning-llms-lora-qlora.tex
...
Mở
colab/Lab21_RUN_ALL.ipynb
Menu Runtime → Change runtime type → T4 GPU → Save.
Xác nhận góc trên có chữ GPU / T4.
...
format thấp → template/mask (NB1), không phải “cần rank lớn hơn”
regression tụt → quên thảm hoạ → trộn 1–5% dữ liệu phổ thông (deck §14.3); với bài nộp core, phân tích hiện tượng vẫn được điểm
target không nhúc nhích → xem wrong_lr trước khi đụng rank
Cả ba ổn nhưng vẫn thua (b) → prompt engineering đã thắng. Kết luận hợp lệ: không nên deploy bản fine-tune này
...
Kênh GitHub
Fork repo lab hoặc tạo repo mới tên lab21-<MSSV>.
Đưa artefact vào đúng path:
lab21-<MSSV>/
├── submission/REPORT.md
...
Copy
Adapter LoRA thường vài chục MB — GitHub chấp nhận. Nếu >100 MB, bật Git LFS cho *.safetensors, hoặc đẩy adapter sang HuggingFace rồi chỉ để link trong REPORT.md / LINKS.md.
Commit, push, kiểm tra trên web: mở được results/verdict.json và submission/REPORT.md.
Nộp URL dạng https://github.com/<user>/lab21-<MSSV>.
...
Tạo tài khoản huggingface.co → Settings → Access Tokens → token Write.
Đăng nhập trên máy hoặc Colab:
pip install -U huggingface_hub
huggingface-cli login
Copy
Tạo model repo public, ví dụ <user>/lab21-<MSSV>-qwen35-triage-vi.
Đẩy adapter và artefact (từ thư mục lab, sau make verify):
...
Copy report (hoặc tóm tắt + đường dẫn submission/REPORT.md) vào model card. Ghi rõ base model (unsloth/Qwen3.5-4B hoặc đúng tier bạn chạy).
Nộp URL dạng https://huggingface.co/<user>/lab21-<MSSV>-qwen35-triage-vi.
